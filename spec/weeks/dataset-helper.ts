...
import { DateTypes } from '../../src/types';
type datesElem = DateTypes[];

const datesSet: datesElem[] = [
...
export const datesAsString = (dates: datesElem[]) => dates.map(
    (elem: datesElem) => [ elem[0], elem[1], elem[2], ]
);

/**
 *  Создаем массив из Date вместо строк
**/
export const datesAsDate = (dates: datesElem[]) => dates.map(
    (elem: datesElem) => [ new Date( elem[0] as string), elem[1], elem[2] ]
);
